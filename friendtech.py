...
#Checks if the user is on the friend.tech api and buys a share if it is
def check_for_user(user_pair):
    try:
        user = user_pair[0]
        print("Checking for user "+user)
        r = requests.get(api_url+user)          #Getting all the users with the similar name from the api
